...
bjscl.render = function(svg, info, dat)
{
	createColaGraph(dat, bjscl.colorPlan);
		...
		dat.colagroups.forEach(function (g) { g.padding = GROUP_PADDING; });
		...
		.data(dat.colalinks, function(d) { return d.id;});

		lt.enter().append("path")
		.attr("class", "link")
		.attr("stroke", function(d){return d.link.type=="filter"?"grey":"blue";})
		.attr("d", connector_cubic);

		lt.exit().remove();

		var nt = svg.selectAll("g.nt")
		.data(dat.nodea, function(d) { return d.fullname;});
		...
		.on("mouseout", mouseOut)
		.attr("transform", function(d){return "translate(400,400)"; })
			;//.call(coke.drag); 
			...
			coke.on("tick", function () {
				...
				nt.attr("transform", function(d) { return "translate(" + (d.x) + "," + (d.y) + ")"; });
		...
		function dragStart(d) {
			...
			ghosts = [1, 2].map(function (i) { return svg.append('circle')
				.attr({
					class: 'ghost',
					cx: d.x,
					cy: d.y,
		...
		function getDragPos(d) {
			var p = getEventPos(), startPos = eventStart[d.fullname];
			return { cx: d.x + p.x - startPos.x, cy: d.y + p.y - startPos.y };
		}
		function drag(d) {
			var p = getDragPos(d);
			ghosts[1].attr(p);
...
function groupMouseOver(d) {
	svg.selectAll(".link")
	.classed("active", function(p) {return p.link.source.pkgname == d.pkg.fullname || p.link.target.pkgname == d.pkg.fullname; }) 
	.classed("passive", function(p){return !(p.link.source.pkgname == d.pkg.fullname || p.link.target.pkgname == d.pkg.fullname); });

	svg.selectAll(".node")
	.classed("active", function(p) {return areNodesRelatedToGroup(p, d.pkg); }) 
	.classed("passive", function(p){return !areNodesRelatedToGroup(p, d.pkg); });

	svg.selectAll(".colagroup")
	.classed("active", function(p) {return p.pkg.fullname == d.pkg.fullname;})
	.classed("passive", function(p) {return p.pkg.fullname != d.pkg.fullname;});
...
function nodeMouseOver(d) {
	svg.selectAll(".link")
    	.classed("active", function(p) { return areNodesRelated(p.link.source, d) && areNodesRelated(p.link.target, d); })
    	.classed("passive",function(p) { return !(areNodesRelated(p.link.source, d) && areNodesRelated(p.link.target, d)); });
